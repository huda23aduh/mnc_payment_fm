...
namespace Payment_job_scheduling_FM
{
    class Program
    {
        ...
        static string mysql_fm_trx_mirror_table = "FM_TRX_PAYMENT";
        static string mysql_fm_trx_problem_table = "FM_TRX_PROBLEM_1";
        static string mysql_prospect_icc_table = "PROSPECT_ICC";

        static string oracle_fm_trx_payment_table = "fm_trx_payment@igpos";
        ...
        static void Main(string[] args)
        {
            ...
            mutex = new Mutex(true, "Payment_job_scheduling_FM");
            ...
            {
                Console.WriteLine("fm_trx_payment start_from = " + max_id_mysql + " <==> until fm_trx_payment =  " + max_trx_id_oracle);
        ...
        static string get_max_trx_id()
        {
            ...
            try
            {
                ...
                command.CommandText = @"SELECT MAX_ROW FROM " + mysql_max_row_trx_table + " WHERE ID='" + 3 + "'";
        ...
        static void read_data_trx_table(int min_seq_id, int max_seq_id)
        {
            ...
            for (int i = min_seq_id + 1; i <= max_seq_id; i++)
            {
                ...
                try
                {
                    ...
                    cmd.Connection = conn;
                    cmd.CommandText = "SELECT * FROM "+ oracle_fm_trx_payment_table +" WHERE ID_PAY = " + ":min_seq_id_param";
                    ...
                    while (dr.Read())
                    {
                        ...
                        if (data[1].Substring(0, 1) == "5" && data[1].Length == 9) //HANDLE IF CUST_NUMBER IS PROSPECT_NUMBER and have icc cust nbr
                        {
                            insert_to_prospect_icc_table_mysql(data);

                            sb.Append("insert to PROSPECT_ICC mysql table#" + data[1] + '#' + data[2] + '#' + data[0] + "#" + DateTime.Now.ToString("dd_M_yyyy_HH:mm:ss") + Environment.NewLine);
                            checking_file_log();
                            write_info_to_log_file();
                        ...
                        if (data[1].Length == 10) //HANDLE IF CUST_NUMBER IS PROSPECT_NUMBER and have no icc cust nbr
                        {
                            insert_to_prospect_icc_table_mysql(data);
                        ...
                        try
                        {
                            ...
                            string the_json = get_request_to_api("http://192.168.177.185:1111/api/iccapi/ap1msky!/payment/doPaymentWithFulfillQuote/" + cust_id + "/" + data[2].ToString() + "/" + merchant_bank);
                            ...
                            else if (parsedJson != null)
                            {
                                ...
                                do_insert_to_fm_trx_payment_mysql(data);
                                #endregion copy data to mysql mirror table

                                sb.Append(data[1] + '#' + data[10] + '#' + data[2] + '#' + merchant_bank + "#" + DateTime.Now.ToString("dd_M_yyyy_HH:mm:ss") + "#success" + Environment.NewLine);
                        ...
                        catch (Exception ex)
                        {
                            ...
                            do_insert_to_fm_trx_problem_mysql(data);
                            #endregion insert data to mysql problem table

                            sb.Append("insert to problem table#" + data[1] + '#' + data[10] + '#' + data[2] + '#' + merchant_bank + "#" + DateTime.Now.ToString("dd_M_yyyy_HH:mm:ss") + "#error catch (Exception ex)" + Environment.NewLine);
                        ...
                        int c = count_data_in_master_table_oracle(data[1]);
        ...
        static void do_insert_to_fm_trx_payment_mysql(string[] data)
        {
            MySqlConnection conn = null;
            MySqlTransaction transaction = null;
            ...
            try
            {
                ...
                command.CommandText = @"INSERT INTO "+mysql_fm_trx_mirror_table+"  (TGLPAY, NOPEL, BAYAR, AUTH_ID, STAN, RC, TRANSDATETIME, PAID, MERCHANT, BANK, NMPEL,  SFROM, STO, CREATED_DATE,  ID_PAY) VALUES (?TGLPAY, ?NOPEL, ?BAYAR, ?AUTH_ID, ?STAN, ?RC, ?TRANSDATETIME, ?PAID, ?MERCHANT, ?BANK, ?NMPEL, ?SFROM, ?STO, ?CREATED_DATE, ?ID_PAY)";

                command.Parameters.AddWithValue("?TGLPAY", data[0]);
                command.Parameters.AddWithValue("?NOPEL", data[1]);
                ...
                command.Parameters.AddWithValue("?CREATED_DATE", data[13].ToString());
                command.Parameters.AddWithValue("?ID_PAY", data[14]);

                command.ExecuteNonQuery();
                transaction.Commit();

                Console.WriteLine("successfully inserted to FM_TRX_PAYMENT");
                sb.Append("successfully inserted to FM_TRX_PAYMENT"  + Environment.NewLine);
        ...
        static void insert_to_prospect_icc_table_mysql(string[] data)
        {
            ...
            try
            {
                ...
                if (data[1].Length == 9) // handle if only have icc cust_nbr and have no 10digit prospect nbr
                {
                    command.Parameters.AddWithValue("?PROSPECT_CUST_NO", "0");
                    command.Parameters.AddWithValue("?ICC_CUST_NO", data[1]);
                }
                if (data[1].Length == 10) // handle if only have icc cust_nbr and have no 10digit prospect nbr
                {
                    command.Parameters.AddWithValue("?PROSPECT_CUST_NO", data[1]);
        ...
        static int count_data_in_master_table_oracle(string cust_id_param)
        {
            //string cs = "User Id=mnc_subscribe;Password=mncsubsd3v;Data Source=192.168.177.102:1521/MNCSVDRC";
        ...
        static void do_update_max_row_id_to_table(string max_quote_id_param)
        {
            ...
            try
            {
                ...
                command.Connection = conn;
                command.Transaction = transaction;
                command.CommandText = @"UPDATE "+ mysql_max_row_trx_table + " SET MAX_ROW='" + max_quote_id_param + "'  WHERE ID='" + 3 + "'";
        ...
        static string get_max_idseq_trx_table_in_oracle()
        {
            ...
            try
            {
                ...
                OracleCommand cmd = new OracleCommand();
                cmd.Connection = conn;
                cmd.CommandText = "SELECT MAX(ID_PAY) FROM "+oracle_fm_trx_payment_table+"";
                cmd.CommandType = CommandType.Text;
                cmd.ExecuteNonQuery();
        ...
        static void checking_file_log()
        {
            ...
            full_path_file = path + "FM_" + file_log_name + ".txt";
        ...
        static string get_new_cust_nbr(string old_cust_nbr)
        {
            ...
            try
            {
                ...
                int c = count_data_in_master_table_oracle(old_cust_nbr);
        ...
        static void do_insert_to_fm_trx_problem_mysql(string[] data)
        {
            MySqlConnection conn = null;
            MySqlTransaction transaction = null;
            ...
            try
            {
                ...
                command.CommandText = @"INSERT INTO "+mysql_fm_trx_problem_table+"  (TGLPAY, NOPEL, BAYAR, AUTH_ID, STAN, RC, TRANSDATETIME, PAID, MERCHANT, BANK, NMPEL,  SFROM, STO, CREATED_DATE,  ID_PAY) VALUES (?TGLPAY, ?NOPEL, ?BAYAR, ?AUTH_ID, ?STAN, ?RC, ?TRANSDATETIME, ?PAID, ?MERCHANT, ?BANK, ?NMPEL, ?SFROM, ?STO, ?CREATED_DATE, ?ID_PAY)";
                ...
                Console.WriteLine("successfully inserted to FM_TRX_PROBLEM");
                sb.Append("successfully inserted to FM_TRX_PROBLEM" + Environment.NewLine);
